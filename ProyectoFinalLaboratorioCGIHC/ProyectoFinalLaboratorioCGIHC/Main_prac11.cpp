...
// Keyframes
float posX =PosIni.x, posY = PosIni.y, posZ = PosIni.z, rotRodIzq = 0, rotRodDer = 0, rotBrazoIzq = 0, rotBrazoDer = 0;
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].rotInc3 = (KeyFrame[playIndex + 1].rotBrazoIzq - KeyFrame[playIndex].rotBrazoIzq) / i_max_steps;
	KeyFrame[playIndex].rotInc4 = (KeyFrame[playIndex + 1].rotBrazoDer - KeyFrame[playIndex].rotBrazoDer) / i_max_steps;
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Sillon
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-95.5f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void animacion()
{
		...
		if (play)
		{
			...
			else
			{
				...
				rotBrazoIzq += KeyFrame[playIndex].rotInc3;
				rotBrazoDer += KeyFrame[playIndex].rotInc4;
